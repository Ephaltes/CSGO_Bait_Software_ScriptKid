...
namespace ScriptKidAntiCheat.Internal
{
    public class MatchInfo
    {
        ...
        public event EventHandler OnMatchEndRound;
        ...
        public int GetCurrentMatchRound(GameProcess gameProcess)
        {
            if(GameRulesProxy != IntPtr.Zero)
            {
                int m_totalRoundsPlayed = gameProcess.Process.Read<int>(GameRulesProxy + Offsets.m_totalRoundsPlayed) + 1;

                if(m_totalRoundsPlayed != RoundNumber && RoundNumber != -1 && RoundNumber != 0)
                {
                    Task.Run(() =>
                    {
                        EventHandler handler = OnMatchEndRound;
                        handler?.Invoke(this, null);
                    });
                }

                return m_totalRoundsPlayed;
            }
